...
// User can read their notes: asking the program to read should display all notes
// in the data.json file. See the example GIF for how they should appear.
...
async function removeNote() {
  try {
    const jsonData = JSON.parse(await readFile('./data.json'));
    const jasonDataNotes = jsonData.notes;
    delete jasonDataNotes[process.argv[3]];
    await writeFile('data.json', JSON.stringify(jsonData, null, 2));
  } catch (error) {
    console.error('Error message', error.message);
  }
...
async function updateNote() {
  try {
    const jsonData = JSON.parse(await readFile('./data.json'));
    const jasonDataNotes = jsonData.notes;
    jasonDataNotes[process.argv[3]] = process.argv[4];
    await writeFile('data.json', JSON.stringify(jsonData, null, 2));
  } catch (error) {
    console.error('Error message', error.message);
  }
